...
module.exports = function(app, io) {
                ...
                User.findById(req.params.id).exec(function(error, friend) {
                    Chat.findOne({
                        ...
                        if (chat) {
                            Message.find({
                                chatId: chat._id
                            }).populate('fromUser').sort('sentDatetime').exec(function(err, messages) {
    ...
    var chat = io.on('connection', function(socket) {
                            ...
                            if (error) {
                                ...
                                if (user === null) {
                                    ...
                                    if (friend._id.toString() === user._id.toString()) {
                                        ...
                                        err.status = 400;
                                        socket.emit('showError', {
                                            err: 'Cannot add yourself as friend'
                                        });
                                    } else {
                                        var fnd1 = {
                                        ...
                                        if (fnd_id_1.indexOf(friend._id.toString()) === -1) {
                                            ...
                                            err.status = 400;
                                            socket.emit('showError', {
                                                err: 'User already in your friend list'
                                            });
                                        }
                                        ...
                                        if (fnd_id_2.indexOf(user._id.toString()) === -1) {
                                            ...
                                            friend.save();
                                            socket.emit('updateProfilFriendListAdd', {
                                                friendId: friend._id.toString(),
                                                friendUsername: friend.username
                                            });
                                            socket.emit('showSuccessMessage', {
                        ...
                        User.findById(data.friendId).exec(function(error, friend) {
                            if (error) {
                                ...
                                if (friend === null) {
                                    ...
                                    friend.save();
                                    socket.emit('updateProfilFriendList', {
        ...
        socket.on('videoChat', function(data) {
            ...
            socket.broadcast.to(this.room).emit('created', {
                senderId: data.senderId,
                user: this.username,
                avatar: this.avatar
            });
            socket.emit('videoChatInvite', {
                senderId: data.senderId,
                avatar: this.avatar
            });
            socket.broadcast.to(this.room).emit('videoChatInvite', {
                senderId: data.senderId,
                avatar: this.avatar
            });
        });
        ...
        socket.on('audioChat', function(data) {
            socket.emit('audiocreated', {
                senderId: data.senderId,
                user: this.username,
                avatar: this.avatar
            });
            socket.broadcast.to(this.room).emit('audiocreated', {
                senderId: data.senderId,
                user: this.username,
                avatar: this.avatar
            });
            socket.emit('audioChatInvite', {
                senderId: data.senderId,
                avatar: this.avatar
            });
            socket.broadcast.to(this.room).emit('audioChatInvite', {
                senderId: data.senderId,
                avatar: this.avatar
            });
        });
        ...
        socket.on('audioChatJoined', function(data) {
            socket.emit('audiojoined', {
                senderId: data.senderId,
                user: this.username,
                avatar: this.avatar
            });
            socket.broadcast.to(this.room).emit('audiojoined', {
                senderId: data.senderId,
                user: this.username,
                avatar: this.avatar
            });
        ...
        socket.on('snapReceived', function(data) {
            chat.in(data.senderId).emit('renderSnap', {
                buf: data.buf,
                senderId: data.senderId,
                user: this.username,
                img: this.avatar
            });
            socket.broadcast.to(this.room).emit('renderSnap', {
                buf: data.buf,
                senderId: data.senderId,
                user: this.username,
                img: this.avatar
